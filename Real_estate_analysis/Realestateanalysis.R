...
r1 = cor(real_estate_data$Bedrooms, real_estate_data$Price)
r2 = cor(real_estate_data$Fireplaces, real_estate_data$Price)
r3 = cor(real_estate_data$Bathrooms, real_estate_data$Price)
#Slope
b1 = r*(sd.x/sd.y)
...
model <- lm(Price~Living.Area + Bedrooms + Bathrooms, data=real_estate_data); summary(model)
...
ci90 = predict(model, newdata = data.frame(Living.Area=newx), level = 0.90, interval = "confidence" )
